...
class UploadHandlerMan_classified_theme
{
    private $options;
    ...
    private function get_file_object($pid) {
		...
		$post = get_post($pid);
            ...
            $file->url = ClassifiedTheme_generate_thumb(wp_get_attachment_url($pid),70,70);
			$file->{'thumbnail_url'} = ClassifiedTheme_generate_thumb(wp_get_attachment_url($pid),70,70);
		...
		require_once(ABSPATH . "wp-admin" . '/includes/file.php');
	  	...
	  	$new_FILES = $this->multiple($_FILES['files']);
					...
					$file->url 	=  $uploaded_file;
						...
						$go = ClassifiedTheme_generate_thumb(wp_get_attachment_url($attach_id),70,70);
...
$upload_handler = new UploadHandlerMan_classified_theme();
