...
function walleto_post_new_product_content_area_function()
{
    ...
    </style><ul class="list-unstyled" id="form_step">';

						echo '<li '.($new_product_step == '1' ? "class='active active_step' " : "").'>'.__("About Your Product", 'Walleto').'</li>';
...
if($new_product_step == "1")
{
    ...
    <div class="p_personal-info">
		<div class="p_personal-info_box">
		<div class="product_list_heading"> <span class="heading_text"><?php echo sprintf(__("Post New Product",'Walleto')); ?></span> </div>
	        </div>
 ...
 <script src="//code.jquery.com/jquery-1.10.2.js"></script>
     <script>
            $(document).ready(function(){            
                $("#pro_name").keyup(function(){
                    var tt = $("#pro_name").val().length;
                    $(".left_aplha").val(50-tt+' Character Left');                   
                });        
            });
        </script>
    <form method="post" action="<?php echo Walleto_post_new_with_pid_stuff_thg($pid, $new_product_step);?>">  
    <ul class="post-new p_form_box">
        <li>
        	<span class="p_form_name"><?php echo __('Your product title', 'Walleto'); ?>:</span>
        	<p>
            <input  type="text" maxlength="50" class="do_input p_text_box" id="pro_name"name="product_title" 
            value="<?php echo (empty($_POST['product_title']) ? 
			($post->post_title == "Auto Draft" ? "" : $post->post_title) : $_POST['product_title']); ?>"  style="width: 73%;"/> <?php do_action('Walleto_step1_after_title_field');  ?>
            <input  type="text" class="left_aplha do_input p_text_box_small" style="margin-left: 5px; border: medium none; padding-right: 10px; padding-left: 10px; width: 24%; text-align: center;" readonly />
            </p>
        </li>
        ...
        <p><input type="text"  name="quant" class="do_input p_text_box_small" 
        ...
        <p><textarea  class="do_input p_textarea" id="product_description"  name="product_description"><?php 
	...
	<li>
		...
		//'.__('Go Back','Walleto').'</a>';
		 ...
		 ?>
        <input class="pink-btn" type="submit" name="product_submit_1" value="<?php _e("Next Step", 'Walleto'); ?> >>" /></p>
        </li>
 ...
 <li>
	...
	?>
 	...
 	<?php /*-------  custom fields  -------- */ ?>
		...
		//color, brand name etc
		for($i=0;$i<count($arr);$i++)
		{
			echo '<li>';
				echo '<span class="p_form_name">'.$arr[$i]['field_name'].$arr[$i]['id'].':</span>';
				echo $arr[$i]['value'];
	...
	?>        
		...
		?>
        <input class="pink-btn" type="submit" name="product_submit_photos" value="<?php _e("Next Step", 'Walleto'); ?> >>" /></p>
        </li>
