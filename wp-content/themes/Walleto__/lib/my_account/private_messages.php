...
?>	
	...
	<div id="content">
	<div class="my_box3">
	<div class="box_title my_account_title"><?php _e("Private Messages","Walleto"); ?></div>
		...
		<div class="box_title my_account_title"><?php _e("Latest Received Messages","Walleto"); ?></div>
			...
			<table class="shopping_cart" width="100%" border="0" cellspacing="0" cellpadding="0">';
					...
					echo '<tbody><tr class="head">';
						...
						echo '<td width="25%">'.__('Subject','Walleto').'</td>';						
						echo '<td>'.__('Date','Walleto').'</td>';
						echo '<td width="20%">'.__('Options','Walleto').'</td>';
						echo '</tr>';
					...
					foreach($r as $row)
					{
						...
						echo '<td width="20%"><a class="red-txt" href="'.$privurl_m.'priv_act=read-message&id='.$row->id.'">'.__('Read','Walleto').'</a> | 
						<a class="red-txt" href="'.$privurl_m.'return=home&priv_act=delete-message&id='.$row->id.'">'.__('Delete','Walleto').'</a></td>';
						echo '</tr>';
		...
		<div class="box_title my_account_title"><?php _e("Private Messages: Inbox","Walleto"); ?></div>
			...
			<table class="shopping_cart" width="100%" border="0" cellspacing="0" cellpadding="0">';
					...
					foreach($r as $row)
					{
						...
						echo '<td class="'.$cls.'">'.$row->subject.'</td>';						
						echo '<td class="'.$cls.'">'.date('d-M-Y H:i:s',$row->datemade).'</td>';
						echo '<td><a class="red-txt" href="'.$privurl_m.'priv_act=read-message&id='.$row->id.'">'.__('Read','Walleto').'</a> | 
						<a class="red-txt" href="'.$privurl_m.'return=inbox&priv_act=delete-message&id='.$row->id.'">'.__('Delete','Walleto').'</a>
						</td>';
						echo '</tr>';
      ...
      <br/> <br/>
       ...
       <?php if($owner == false): ?>
     <a href="<?php echo $privurl_m; ?>priv_act=delete-message&<?php echo 'id='.$row->id.'&return='.$_GET['return']."&confirm_message_deletion=yes&attr=return";
	?>" class="nice_link"><?php _e("Confirm Deletion",'Walleto'); ?></a>
	<script>
		if(document.location.href.indexOf('attr=return') === -1){ 
	    	
		}else{			
			window.location = "<?php echo $privurl_m; ?>priv_act=inbox";
		}
	</script>
	<?php
	if($_GET['confirm_message_deletion'] == 'yes'):
	$wpdb->query($wpdb->prepare("DELETE FROM ".$wpdb->prefix."walleto_pm where id='$id' AND (user='$myuid' OR initiator='$myuid')" , $id));
        ...
        <div class="my_box3">            
            	<div class="box_title my_account_title">
				 	<?php
				 	$url = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
				 	if (strpos($url,'paythis')) {
						echo sprintf(__("To obtain payment details, please send a private message to: %s","Walleto"), $user->user_login); 
					} else {
					    echo "Send a private message"; 
					}
				 	?>
            	</div>
                <div class="box_content">  
                ...
                <td><input class="p_text_box" name="to" type="text" value="" /></td>
                </tr>
                <?php endif; ?>
                
                <tr>
                <td width="140"><?php _e("Subject", "Walleto"); ?>:</td>
                <td><input class="p_text_box" name="subject" type="text" value="<?php echo $subject; ?>" /></td>
                </tr>
                
                <tr>
                <td valign="top"><?php _e("Message", "Walleto"); ?>:</td>
                <td><textarea class="p_textarea" name="message" rows="6" cols="50"></textarea></td>
                </tr>
                ...
                <td></td>
                 ...
                 <tr>
                <td width="140">&nbsp;</td>
                <td><input class="p_save_btn" name="send" type="submit" value="<?php _e("Send Message",'Walleto'); ?>" /></td>
                </tr>
