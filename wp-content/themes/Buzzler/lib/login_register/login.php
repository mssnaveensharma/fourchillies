...
function Buzzler_sitemile_filter_ttl($title){ global $real_ttl; return $real_ttl." - ".get_bloginfo('sitename');}

function Buzzler_do_login_scr()
		{
		...
		do_action( 'login_init' );
		  ...
		  switch($_REQUEST["action"])
		  {
			...
			add_filter( 'wp_title', 'Buzzler_sitemile_filter_ttl', 10, 3 );	
			  ...
			  //Any differences will be noted with end of line comments. 
				 ...
				 */
				 if ( !empty( $_REQUEST['redirect_to'] ) ) {
					$redirect_to = get_permalink(get_option('Buzzler_my_account_page_id'));
				 } else {
					$redirect_to = $_REQUEST['redirect_to'];
				 }
				 
				 if(empty($redirect_to)) $redirect_to = get_permalink(get_option('Buzzler_my_account_page_id'));
