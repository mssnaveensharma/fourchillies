...
add_action('widgets_init', 'buzzler_register_browse_by_category_widget_auction');
function buzzler_register_browse_by_category_widget_auction() {
	register_widget('Buzzler_browse_by_category');
}

class Buzzler_browse_by_category extends WP_Widget {

	function Buzzler_browse_by_category() {
		...
		$this->WP_Widget( 'browse-by-category', 'Buzzler - Browse by Category', $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		...
		$terms_k 	= get_terms("listing_cat","parent=0&hide_empty=0");
		$terms 		= get_terms("listing_cat","parent=0&hide_empty=0".$jk);
	...
	//=========================================================================
		...
		$contor = 0;
		 ...
		 if ( $count > 0 ){
			...
			$cnt	= 1;
			   	...
			   	$terms2 = get_terms("listing_cat","parent=".$term->term_id."&hide_empty=0");
				

				$mese = '';
				
					$mese .= '<ul>';
					$link = get_term_link($term->slug,"listing_cat");
			   ...
			   if($terms2)
				{
					...
					foreach ( $terms2 as $term2 ) 
					{
						...
						$tt = Buzzler_get_custom_taxonomy_count('auction',$term2->slug);
		       			///$total_ads += $tt;
						$link = get_term_link($term2->slug,"listing_cat");
						$mese2 .= "<li><a href='".$link."'>" . $term2->name." ". ($show_me_count == true ? "(".$tt.")" : "")."</a></li>
						";
						
						
						$terms3 = get_terms("listing_cat","parent=".$term2->term_id."&hide_empty=0");
						...
						if($terms3)
						{
							...
							foreach ( $terms3 as $term3 ) 
							{
								...
								///$total_ads += $tt;
								$link = get_term_link($term3->slug,"listing_cat");
		...
		if($disp_btn == 1)
		{
				echo '<div class="see-more-tax"><b><a href="'.get_permalink(get_option('Buzzler_all_cats_id')) .'">'.__('See More Categories','Buzzler').'</a></b></div>';		
		}		
    ...
    overflow:auto;border:1px solid #ccc">
	 ...
	 $terms = get_terms("listing_cat","parent=0&hide_empty=0");
	 foreach ( $terms as $term ) {
