...

{
...

	{
		...
		$arr[$i]['value']  = '<textarea class="do_input p_text_box" type="text" name="custom_field_value_'.$ids.'">'.(isset($_POST['custom_field_value_'.$ids]) ? $_POST['custom_field_value_'.$ids] : $teka ).'</textarea>';
...
?>
            ...
            <li>
	        <div class="product_block_top dj product_block_light_strip"></div>              
                <div class="product_block_middle allproduct">
                <a title="<?php the_title();?>" href="<?php the_permalink();?>"><?php
                    echo walleto_get_first_post_image(get_the_ID(),184,161,$image_thing_tags, 1);?>
                </a>
                <div class="product-content">
		<a class="product_name dj" title="<?php the_title();?>" href="<?php the_permalink();?>"><?php $ttl = get_the_title();
                        $xx  = 24;
                       if (strlen($ttl) > $xx)
                       echo substr($ttl, 0, $xx);
                       else
...

{
	...
	{	
		...
		$shop_paypal =  trim($_POST['shop_paypal']);
	
		$shop_facebook 			= trim($_POST['shop_facebook']);

		$shop_twitter 			= trim($_POST['shop_twitter']);
		...
		update_post_meta($pid,'shop_paypal', $shop_paypal);
...

{
    ...
    <td width="130"><?php _e('Shop Paypal Email:','Walleto'); ?></td>

    <td><input type="text" size="45" value="<?php echo get_post_meta($pid, 'shop_paypal', true); ?>" name="shop_paypal" /></td>
	...
	?>
        ...
        <ul id="side_menu_dashboard_wrapper">
                ...
                <ul id="my-account-admin-menu">
                    ...
                    <li><a <?php echo $active_messages; ?> href="<?php echo get_permalink($messages_page); ?>"><i class="icon-double-angle-right"></i><?php _e("Private Messages",'Walleto'); 
	                    global $current_user;
						get_currentuserinfo();
						$rd = Walleto_get_unread_number_messages($current_user->ID);
						if($rd > 0) echo ' <span class="nots_nots">'.$rd.'</span>';
